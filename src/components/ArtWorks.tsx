...
export const ArtWorks: React.FC = () => {
  ...
  return (
    <div className="w-full h-[750px] relative bg-cyan-dark/[0.03] rounded-[3rem] border-4 border-cyan-main/10 shadow-inner overflow-hidden flex flex-col justify-center gap-12 py-16">
      ...
      <div className="absolute top-8 left-10 z-50 pointer-events-none">
        <h3 className="text-xl font-black text-cyan-dark uppercase tracking-tighter flex items-center gap-3">
          <span className="w-10 h-1 bg-cyan-main rounded-full" />
          {t("图库展示", "ART GALLERY")}
        </h3>
      </div>
      ...
      <div className="flex items-center h-64">
        <motion.div 
          className="flex gap-10 px-10 items-center h-full"
          animate={{ x: ["0%", "-33.333%"] }}
        ...
        >
          {row1.map((art, index) => (
            <motion.div
              ...
              className="h-full flex-shrink-0 shadow-xl rounded-xl overflow-hidden bg-white/50 backdrop-blur-sm border-2 border-cyan-light/10"
            >
              <img 
                src={art.image} 
                alt="" 
      ...
      {/* Edge Fades */}
      <div className="absolute inset-y-0 left-0 w-48 bg-gradient-to-r from-white/20 via-white/5 to-transparent pointer-events-none z-20" />
      <div className="absolute inset-y-0 right-0 w-48 bg-gradient-to-l from-white/20 via-white/5 to-transparent pointer-events-none z-20" />
    </div>
  );
};
